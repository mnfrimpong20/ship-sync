...
export function mountInsights(r: Router, d: Deps) {
  ...
  r.get('/shipper/overview', wrap(async (req, res) => {
    ...
    for (const s of late.slice(0, 3)) attention.push({ kind: 'shipment', severity: 'critical', title: `${s.ref} is past its ETA (${dateOnly(s.eta)})`, body: `${statusLabels[s.status as ShipmentStatus]} · ${s.customer}. Update the status or the ETA so the customer isn’t guessing.`, to: '/dashboard/shipper?view=shipments' })
    ...
    for (const s of stale.slice(0, 2)) attention.push({ kind: 'shipment', severity: 'warning', title: `${s.ref} hasn’t moved in ${Math.floor((now.getTime() - new Date(s.last_event).getTime()) / DAY)} days`, body: `Still “${statusLabels[s.status as ShipmentStatus]}”. Add an update so tracking stays honest.`, to: '/dashboard/shipper?view=shipments' })
