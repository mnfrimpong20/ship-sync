...
export function mountInsights(r: Router, d: Deps) {
  ...
  r.get('/shipper/overview', wrap(async (req, res) => {
    ...
    for (const l of staleLeads.slice(0, 3)) attention.push({ kind: 'lead', severity: 'warning', title: `Lead waiting ${Math.floor((now.getTime() - new Date(l.created_at).getTime()) / DAY)}d: ${l.origin} → ${countryByCode(l.destination)?.name ?? l.destination}`, body: `${l.contact_name} · ${l.quote_count} competing quote${l.quote_count === 1 ? '' : 's'}. Customers see your response time.`, to: '/dashboard/leads', at: iso(l.created_at) })
