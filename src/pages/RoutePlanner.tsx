                    ...
                    return (
                      <li key={s.id} className={`rounded-xl border px-3 py-2.5 ${focus === s.id ? 'border-gold/60' : 'border-border'} ${s.status === 'done' ? 'opacity-70' : ''}`}>
                        <div className="flex items-start gap-3">
                          <button type="button" onClick={() => setFocus(s.id)} className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-xs font-bold focus-ring ${s.status === 'done' ? 'bg-green/20 text-green' : s.status === 'skipped' ? 'bg-surface-2 text-text-muted' : 'bg-gold text-on-accent'}`} aria-label={`Show stop ${i + 1} on the map`}>{s.status === 'done' ? <Check size={13} /> : i + 1}</button>
